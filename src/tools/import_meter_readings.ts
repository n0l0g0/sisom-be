...
async function main() {
  ...
  const month = overrideMonthArg
    ? parseInt(overrideMonthArg.split('=')[1], 10)
    : inferred.month;
  const year = overrideYearArg
    ? parseInt(overrideYearArg.split('=')[1], 10)
    : inferred.year;
  ...
  for (let i = startIdx; i < rows.length; i++) {
    ...
    if (!room) {
      ...
      await prisma.room.update({
        where: { id: room.id },
        data: {
          floor: Number.isFinite(floorNum) ? floorNum : room.floor,
          buildingId: building.id,
        },
      });
    }
    ...
    if (Number.isFinite(waterPrev) || Number.isFinite(elecPrev)) {
      await prisma.meterReading.upsert({
        where: {
          roomId_month_year: { roomId: room.id, month: prevM, year: prevY },
        },
        update: {
